...
//
extern bool OSAtomicCompareAndSwapPtrBarrier(void *oldValue, void *newValue, void *volatile *theValue);
// MARK: 获取一个 pthread 对应的 RunLoop。
...
// MARK: __CFRunLoopServiceMachPort 内部函数
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode)
{
    ...
    do
    {
        ...
        if (MACH_PORT_NULL != dispatchPort && !didDispatchPortLastTime) // 根本不会执行 didDispatchPortLastTime == true
        {
#if DEPLOYMENT_TARGET_MACOSX || DEPLOYMENT_TARGET_EMBEDDED || DEPLOYMENT_TARGET_EMBEDDED_MINI
...
// MARK: 用指定的Mode启动，允许设置RunLoop超时时间
